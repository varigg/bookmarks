...
- ✅ Phase 5: Configuration Management (Externalized constants)
- ✅ Phase 6: Improve Test Suite (Mocking, unit tests, coverage)
...
- **Status:** ✅ Completed
- **Changes:** Created `bookmarks/config.py` with environment variable overrides and sensible defaults.

### 5.2 Use Config in Services ✅

- **Status:** ✅ Completed
- **Changes:** Services now use `bookmarks.config` for provider settings and paths.
...
- **Status:** ✅ Completed
- **Changes:** Implemented mocking for LLM providers and content extractors in `tests/`.

### 6.2 Add Unit Tests for New Modules ✅

- **Status:** ✅ Completed
- **Changes:** Added `test_bookmark_service.py`, `test_content_extractor.py`, `test_llm_factory.py`, and `test_llm_service.py`.

### 6.3 Add Integration Tests ✅

- **Status:** ✅ Completed
- **Changes:** Standardized `test_app.py` and `test_autofill_route.py` to use per-test storage.
